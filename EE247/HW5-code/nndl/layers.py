...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    #         the 'cache' variable.
    # ================================================================ #
    # calculating the sample_mean and sample_var
    sample_mean = np.mean(x, axis=0)
    sample_var = np.var(x, axis=0)    
    # calculating the running_mean and runing_var    
    running_mean = momentum*running_mean + (1-momentum)*sample_mean
    running_var = momentum*running_var + (1-momentum)* sample_var    
    ...
    out = gamma*x_n + beta    
    # store in cache
    cache = (mode, x, sample_var, eps, gamma, x_n, out, sample_mean)
    # ================================================================ #
    # END YOUR CODE HERE
    # ================================================================ #
    ...
    x_t = (x - running_mean)/np.sqrt(running_var + eps)
    t = 1/np.sqrt(running_var + eps)
    # store in out
    out = gamma*x_t + beta    
    # store in cache
    cache = (mode, x, eps, gamma, x_t, out, t)
    # ================================================================ #
    # END YOUR CODE HERE
    # ================================================================ #
...
def batchnorm_backward(dout, cache):
  ...
  # training mode
  if(mode == 'train'):
    # deriving the necessary variables
    mode, x, sample_var, eps, gamma, x_n, out, sample_mean = cache
    M, D = x.shape
    # backpropagating dbeta
    dbeta = np.sum(dout,axis = 0)
    # keep backpropagating
    db = gamma*dout
    # backpropagating dgamma
    b = x_n
    dgamma = np.sum(b*dout, axis = 0)
    # keep backpropagating
    d = 1/np.sqrt(sample_var + eps)
    dc = d*db
    # derive dx
    dvar = (-0.5)*b*d*d*(db)
    dx = d*db + (1/M)*(-d)*np.sum(db, axis = 0) + 2*((x - sample_mean)/M)*(np.sum(dvar, axis = 0))
  elif(mode == 'test'):
    # deriving the necessary variables
    mode, x, eps, gamma, x_t, out, t = cache
    # deriving dbeta
    dbeta = np.sum(dout,axis = 0)
    # keep backpropagating
    b = x_t
